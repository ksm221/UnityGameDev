...
public class ReverseCamTrig : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        if (other.CompareTag("Player") && !enteredBossRoom)
        {
            ...
            mainCameraGameObject.SetActive(true); // Disable the Main Camera GameObject
            bossCameraGameObject.SetActive(false);  // Enable the Boss Camera GameObject
    ...
    private void OnTriggerExit2D(Collider2D other)
    {
        if (other.CompareTag("Player") && enteredBossRoom && Time.time > lastEnterTime + exitCooldown)
        {
            ...
            mainCameraGameObject.SetActive(false);  // Enable the Main Camera GameObject
            bossCameraGameObject.SetActive(true); // Disable the Boss Camera GameObject
